...
// 图片下载函数 - 支持防盗链
async function downloadImageWithAntiHotlink(url, context) {
    try {
        const { fetch } = context;
        // 设置防盗链绕过请求头
        const headers = {
...
block_basekit_server_api_1.basekit.addAction({
    ...
    formItems: [
        ...
        },
        {
            ...
            label: '目标附件字段',
            required: true,
            component: block_basekit_server_api_1.Component.Attachment,
            componentProps: {
    ...
    execute: async function (args, context) {
        try {
            ...
            // 上传到附件字段
            const attachments = await (0, block_basekit_server_api_1.uploadAttachments)([
                {
                    name: filename,
                    file: imageBuffer
                }
    ...
    resultType: {
        ...
        properties: {
            ...
            fileSize: {
                type: block_basekit_server_api_1.ParamType.Number,
                label: '文件大小(字节)'
            },
            attachments: {
                type: block_basekit_server_api_1.ParamType.Array,
                structureType: block_basekit_server_api_1.StructureType.Attachment,
                label: '附件'
            },
            error: {
                type: block_basekit_server_api_1.ParamType.String,
